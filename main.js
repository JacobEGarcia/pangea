...
const glbs = {};
{
  ...
  const ids = ['trex','trike','brach','stego','raptor','anky','para','lystro','ptero'];
  await Promise.all(ids.map(id => new Promise(res => {
    loader.load('models/' + id + '.glb', g => { glbs[id] = g; res(); }, undefined, () => res());
  })));
}

function makeAnimalParts(ex) {
  const g = glbs[ex.id];
  if (!g) return { kind: 'proc', parts: buildCreature(ex.spec), root: null };
  const inst = SkeletonUtils.clone(g.scene);
...
for (const ex of EXHIBITS) {
  ...
  const n = ex.count || 1;
  for (let i = 0; i < n; i++) {
    const ap = makeAnimalParts(ex);
    if (ap.kind === 'proc') ap.root = ap.parts.root;
    ap.root.scale.setScalar(ex.scale * (n > 1 ? 0.9 + Math.random() * 0.2 : 1));
    scene.add(ap.root);
    animals.push({
      ap, ex,
